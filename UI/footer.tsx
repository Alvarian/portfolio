...
const Footer: React.FC<any> = () => {
  ...
  const handleRenderLinks = () => {
    return sectionData.map((section: Content, index: number) => {
      return (<a key={index} className="link link-hover">{section.title}</a> )
    })
  }

  return (
    <footer className="footer footer-center p-10 bg-black text-base-content bg-no-repeat bg-cover bg-center bg-fixed relative" style={styles}>
